...
void MyGLWidget::initializeGL ()
{
  ...
  calcCapsaCont(model, false);
  calcCapsaCont(model2, true);
  calcCapsaScene();
  calcCenterScene();
  calcRadiScene();
...
void MyGLWidget::initializeCamera() {
  ...
  //View
  vrp = sceneCenter;
  obs = sceneCenter + glm::vec3(0,0,distancia);
  up = glm::vec3(0,1,0);
...
//  glViewport (0, 0, ample, alt);
  ...
  modelTransformH(glm::vec3(2,0,2), glm::vec3(0,1,0), 0, centerM);
  glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat) * model.faces().size() * 3);

  // Patricio 2
  if (!RadioP) {
    glBindVertexArray (VAO_model);
    modelTransformH(glm::vec3(0,0,0), glm::vec3(0,1,0), float(M_PI/2), centerM);
    glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat) * model.faces().size() * 3);
  }
  else {
    glBindVertexArray (VAO_model2);
    modelTransformH(glm::vec3(0,0,0), glm::vec3(0,1,0), float(M_PI/2), centerLM);
    glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat) * model2.faces().size() * 3);
  }
  
  // Patricio 3
  glBindVertexArray (VAO_model);
  modelTransformH(glm::vec3(-2,0,-2), glm::vec3(0,1,0), float(M_PI), centerM);
  glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat) * model.faces().size() * 3);
...
void MyGLWidget::modelTransformH(glm::vec3 pos, glm::vec3 rotat, float alfa, glm::vec3 cc) 
{
  // Matriu de transformació de model
  glm::mat4 transform (1.0f);

  transform = glm::translate(transform, pos);
  transform = glm::rotate(transform, alfa, rotat);

  transform = glm::scale(transform, glm::vec3(escala));
  transform = glm::scale(transform, glm::vec3(scale));
  transform = glm::rotate(transform, rte, glm::vec3(0,1,0));
  transform = glm::translate(transform, -cc);
...
void MyGLWidget::projectTransform() {
  glm::mat4 Proj = glm::perspective(FOV + FOVoffset, raw, zNear, zFar);
  if (ort) Proj = glm::ortho(-2.5f*raw*(1+FOVoffset), 2.5f*raw*(1+FOVoffset), 0.0f*(1+FOVoffset), 4.0f*(1+FOVoffset), zNear, zFar);
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  float diffX = e->x() - xold;
  float diffY = e->y() - yold;
...
void MyGLWidget::creaBuffers2() 
{
  ...
  glBindBuffer(GL_ARRAY_BUFFER, VBO_model[0]);
  glBufferData(GL_ARRAY_BUFFER, 
               sizeof(GLfloat) * model2.faces().size() * 3 * 3,
               model2.VBO_vertices(), 
               GL_STATIC_DRAW);
...
void MyGLWidget::calcCapsaCont(Model m, bool lm) {
  xmin = xmax = m.vertices()[0];
  ymin = ymax = m.vertices()[1];
  zmin = zmax = m.vertices()[2];

  for (unsigned int i = 3; i < model.vertices().size(); i += 3) {
    xmin = std::fmin(m.vertices()[i+0], xmin);
    xmax = std::fmax(m.vertices()[i+0], xmax);

    ymin = std::fmin(m.vertices()[i+1], ymin);
    ymax = std::fmax(m.vertices()[i+1], ymax);

    zmin = std::fmin(m.vertices()[i+2], zmin);
    zmax = std::fmax(m.vertices()[i+2], zmax);
  }
  // sceneMax = glm::vec3(xmax, ymax, zmax);
  // sceneMin = glm::vec3(xmin, ymin, zmin);

  escala = 1.0/(xmax-xmin);
  if (!lm) centerM = glm::vec3((xmin+xmax)/2, ymin, (zmin+zmax)/2);
  else centerLM = glm::vec3((xmin+xmax)/2, ymin, (zmin+zmax)/2);
