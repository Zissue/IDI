...
void MyGLWidget::initializeGL ()
{
  ...
  colFocus = glm::vec3(1, 1, 0.);  // color de la llum
  glUniform3fv(colFocusLoc, 1, &colFocus[0]);

  posFocus = glm::vec3(1, 1, 1);        // posicio de la llum en SCA
  posFocusV = glm::vec3(1, 1, 1);
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::scale(TG, glm::vec3(escala*sc, escala*sc, escala*sc));
...
{
  View = glm::mat4(0.f);  // Matriu de posició i orientació
...
void MyGLWidget::Focusllum() {
  glm::vec3 pF;
  if (sF) pF = posFocus;
  else pF = glm::vec3(View * glm::vec4(posFocusV, 1.f));
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_K: { // Moure focus de llum cap a X-
      if (sF) posFocus.x -= 0.15;
      else posFocusV.x -= 0.15;
      Focusllum();
      break;
    }
    case Qt::Key_L: { // Moure focus de llum cap a X+
      if (sF) posFocus.x += 0.15;
      else posFocusV.x += 0.15;
      Focusllum();
      break;
    }
    case Qt::Key_F: { // Switch between scene focus or view focus
      sF = !sF;
      Focusllum();
      break;
    }
    case Qt::Key_B: { // enable back-face culling
      culling = !culling;
      if (culling) glEnable(GL_CULL_FACE);
      else glDisable(GL_CULL_FACE);
      break;
    }
    case Qt::Key_P: { // switch between lights off and on
      lightsoff = !lightsoff;
      if (lightsoff) colFocus = glm::vec3(0,0,0);
