...
//  glViewport (0, 0, ample, alt);
  ...
  modelTransformH(glm::vec3(2,0,2), glm::vec3(0,1,0), 0);
  glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat) * model.faces().size() * 3);

  // Patricio 2
  glBindVertexArray (VAO_model);
  modelTransformH(glm::vec3(0,0,0), glm::vec3(0,1,0), float(M_PI/2));
  glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat) * model.faces().size() * 3);

  // Patricio 3
  glBindVertexArray (VAO_model);
  modelTransformH(glm::vec3(-2,0,-2), glm::vec3(0,1,0), float(M_PI));
  glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat) * model.faces().size() * 3);
...
void MyGLWidget::modelTransformH(glm::vec3 pos, glm::vec3 rotat, float alfa) 
{
